...
func buildStateSignatureID(sessionID, dataset, stage, filtersKey string) uuid.UUID {
	basis := strings.Join([]string{strings.TrimSpace(sessionID), strings.TrimSpace(dataset), strings.TrimSpace(stage), strings.TrimSpace(filtersKey)}, "|")
...
// computeSchemaVersionID hashes (sorted cols | n | filtersKey) into short id
func computeSchemaVersionID(cols []string, n int, filtersKey string) string {
	c := make([]string, len(cols))
	copy(c, cols)
	sort.Strings(c)
	joined := strings.Join(c, "|") + fmt.Sprintf("|%d|%s", n, strings.TrimSpace(filtersKey))
	sum := sha256.Sum256([]byte(joined))
	return fmt.Sprintf("%x", sum[:4])
}

// extractFiltersFromCode finds simple boolean-index filters in pandas-like code.
// Supports patterns like df[df.Col == 'M'], df[df['Col'] > 50], and conjunctions with &/|.
// Only keeps predicates with literal RHS (number or quoted string).
func extractFiltersFromCode(code string) []string {
	code = strings.ReplaceAll(code, "\n", " ")
	// Grab content inside df[...] occurrences
	...
	for _, m := range matches {
		...
		parts := regexp.MustCompile(`[&|]`).Split(cond, -1)
		for _, p := range parts {
			p = strings.TrimSpace(p)
			if p == "" {
				continue
			}
			if am := atomRe1.FindStringSubmatch(p); len(am) > 0 {
				col := strings.TrimSpace(am[1])
				op := am[2]
				rhs := strings.TrimSpace(am[3])
				// Strip quotes around string literal
				if (strings.HasPrefix(rhs, "\"") && strings.HasSuffix(rhs, "\"")) || (strings.HasPrefix(rhs, "'") && strings.HasSuffix(rhs, "'")) {
					rhs = strings.Trim(rhs, "'\"")
				} else if !regexp.MustCompile(`^\d+(?:\.\d+)?$`).MatchString(rhs) {
					// Non-literal RHS; skip
					continue
				}
				preds = append(preds, fmt.Sprintf("%s%s%s", col, op, rhs))
				continue
			}
			if am := atomRe2.FindStringSubmatch(p); len(am) > 0 {
				col := strings.TrimSpace(am[1])
				op := am[2]
				rhs := strings.TrimSpace(am[3])
				if (strings.HasPrefix(rhs, "\"") && strings.HasSuffix(rhs, "\"")) || (strings.HasPrefix(rhs, "'") && strings.HasSuffix(rhs, "'")) {
...
func makeFiltersKey(filters []string) string {
	if len(filters) == 0 {
		return ""
	}
...
func (r *RAG) ingestStateCard(ctx context.Context, sessionID string, baseMeta map[string]string, code string, toolContent string) {
	...
	// New document ID per version (so history is preserved)
	docID := uuid.New()
	...
	// Supersede prior state with same signature if schema version changed
	if existingID, existingContent, existingMeta, err := r.store.FindStateDocumentWithFilters(ctx, sessionID, dataset, stage, filtersKey); err == nil && existingID != uuid.Nil {
		prevVer := strings.TrimSpace(existingMeta["schema_version_id"])
		if prevVer != "" && prevVer != schemaVersionID {
			if r.logger != nil {
				r.logger.Info("Superseding prior state due to schema version change",
					zap.String("dataset", dataset), zap.String("stage", stage), zap.String("filters_key", filtersKey),
					zap.String("prev_version", prevVer), zap.String("new_version", schemaVersionID))
			}
			// Update previous doc metadata to superseded
			newPrevMeta := make(map[string]string, len(existingMeta))
			for k, v := range existingMeta {
				newPrevMeta[k] = v
			}
			newPrevMeta["state_status"] = "superseded"
			newPrevMeta["superseded_by"] = docID.String()
			// Preserve content hash if we can compute
			prevHash := HashContent(NormalizeForHash(existingContent))
			if _, err := r.store.UpsertDocument(ctx, existingID, existingContent, newPrevMeta, prevHash); err != nil {
				if r.logger != nil {
					r.logger.Warn("Failed to mark previous state as superseded", zap.Error(err))
				}
			}
			// Link parent in new doc
			md["parent_state_id"] = existingID.String()
		}
	}
