...
function renderAndProcessContent(contentDiv, content) {
    ...
    // Remove stray paragraphs that contain only backticks/whitespace (handles `, ``, ```)
    // These can appear if a partial fence slipped through mid-stream; not meaningful content
    contentDiv.querySelectorAll('p').forEach((p) => {
        const t = (p.textContent || '').trim();
        const withoutTicks = t.replace(/`/g, '').trim();
        const tickCount = (t.match(/`/g) || []).length;
        if (withoutTicks.length === 0 && tickCount <= 3) {
            p.remove();
        }
    });
    ...
    contentDiv.querySelectorAll('pre:not([data-collapsible="true"])').forEach((preElement, index) => {
        ...
        // Skip empty non-python blocks (prevents empty Execution Output in UI)
        if (!isPython) {
